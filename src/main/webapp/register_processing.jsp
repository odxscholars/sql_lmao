...
<jsp:useBean id="A" class="assetManagement.assets" scope="session"/>
    <%
        ...
        if (status == 1){ %>
        <h1>Asset Registration Successful </h1><br>
        ...
        <h1>Asset Registration failed </h1><br>
        <%
        }
        %>

        <%--/*System.out.println(A.type_asset);
        System.out.println("Enclosing Asset: " + A.enclosing_asset);
        if (!(A.type_asset.equals("P")) && (A.enclosing_asset != ---%><%--1)){--%>
        <%--<h1>Asset Registration failed </h1><br>
        <h2>Cause: Asset is a property but enclosing asset exists!</h2> <br>
        <h3>Please go back to form</h3> <br>--%>
        <%--<%
